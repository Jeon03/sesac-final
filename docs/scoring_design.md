...
> 최종 업데이트: 2026-03-30
> 주요 변경: 리뷰 카테고리 v2 세분화 적용, 정규화 방식 독립 범위로 변경, 가중치 조정, METRIC_RANGES 실측치 기반 재조정
...
```python
METRIC_RANGES = {
    "size":          (0.0,   30.0),   # 십억 달러 기준 (US ~24B, JP ~0.9B)
    "growth":        (0.0,    0.08),  # CAGR 실측 기준 상한 조정 (15% → 8%)
    "kbeauty":       (0.0,    0.50),  # K-뷰티 점유율 최대 50% (JP ~42%, US ~4%)
