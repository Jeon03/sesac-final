...
반드시 아래 JSON 형식으로만 반환하세요:
{{
  "ingredients": ["원문에서 트렌드 성분으로 주목받는 상위 5개 (선정 기준: 언급 빈도 높은 것 → 성장·급증 표현과 함께 언급된 것 → 전문가·브랜드가 핵심 성분으로 강조한 것 순. 한국 통용 표기)"],
  "formulations": ["원문에 언급된 모든 제형 리스트 (예: 세럼, 크림, 토너, 에센스)"],
  "functions": ["원문에 언급된 모든 기능·효능 리스트 (예: 보습, 미백, 안티에이징, 진정)"],
  "rising_keywords": ["소비자 스킨케어 구매·사용 행동 트렌드 키워드 (예: 스키니멀리즘, 멀티태스킹, 선케어 일상화). 성분·기능·마케팅 용어(클린뷰티, AI, 지속가능성 등)는 제외. 없으면 빈 리스트"],
...
온라인·오프라인 구분 없이 원문에 언급된 모든 유통 채널을 하나의 리스트로 추출합니다.
원문에 명시되지 않은 채널은 절대 추가하지 마세요. 원문에 없으면 빈 리스트([])로 반환하세요.

채널명은 원어 그대로 사용하세요 (예: Amazon, Sephora, Ulta Beauty, Target, Qoo10, Rakuten).
...
{{
  ...
  "description": "보고서용 설명문 2~3문장 (주요 채널·온오프라인 비중·진출 시사점 포함)",
  "source_quote": "유통 채널 관련 원문 발췌 (원어 그대로)"
}}

- channels: 원문에 언급된 채널만 포함 (최대 10개). 원문에 없는 채널은 절대 추가하지 마세요.
- type: 해당 채널이 온라인인지 오프라인인지, 둘 다인지 표기.
""" + _COMMON_RULES
